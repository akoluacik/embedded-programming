...
TimerHandle_t xAutoReload;
uint16_t period = 500;
int main(void)
{

  BaseType_t xAutoReloadStarted;
  ...
  /* Create the one shot timer, storing the handle to the created time in xAutoReload */
  xAutoReload    = xTimerCreate(
		  	  	    "AutoReload",
  ...
  if ( ( NULL != xAutoReload ))
  {
	  ...
	  xAutoReloadStarted     = xTimerStart( xAutoReload, 0 );
...
void prvTimerCallback(TimerHandle_t xTimer)
{
	...
	TickType_t tickCount = pdMS_TO_TICKS(period);
	...
	xTimerChangePeriod(xTimer, tickCount, 0);
	period = (period + 500) > 2500 ? 500 : (period + 500); // to have a period less than 2.5s
